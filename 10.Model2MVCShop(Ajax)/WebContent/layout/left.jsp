		 ...
		 $(function() {
			...
			//==> DOM Object GET 3가지 방법 ==> 1. $(tagName) : 2.(#id) : 3.$(.className)
		 	...
		 	$( ".Depth03:contains('판매상품등록')" ).on("click" , function() {
		 		$(parent.frames["rightFrame"].location).attr("href","../product/addProductView.jsp");
			}); 
		 	
		 	$( ".Depth03:contains('판매상품관리')" ).on("click" , function() {
		 		$(parent.frames["rightFrame"].location).attr("href","/product/listProduct?menu=manage");
			}); 
		 	
		 	$( ".Depth03:contains('상 품 검 색')" ).on("click" , function() {
		 		$(parent.frames["rightFrame"].location).attr("href","/product/listProduct?menu=search");
			}); 
		 	
		 	$( ".Depth03:contains('구매이력조회')" ).on("click" , function() {
		 		$(parent.frames["rightFrame"].location).attr("href","/purchase/listPurchase");
			});
